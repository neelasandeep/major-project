...
$que="SELECT CATEGERY,count(*) as number FROM problems where DATEDIFF('$dat',TIME)>2 GROUP BY CATEGERY ";
...
if(!$que)
{
 ...
 if (isset($_POST['Submit'])) {
  ...
  if ($_POST['prob']=="Water") {
    header("location:CMWater.php");
  }
  if ($_POST['prob']=="Electricity") {
    header("location:CMElectricity.php");
  }
  if ($_POST['prob']=="food_agriculture") {
    header("location:CMFagri.php");
  }
  if ($_POST['prob']=="Transport") {
    header("location:CMTransport.php");
  }
